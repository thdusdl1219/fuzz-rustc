...
if [ ! -d rust ]; then
    git clone https://github.com/thdusdl1219/rust.git --branch fuzz
fi
...
export LLVM_PROFILE_FILE=$PWD/test.profraw

mkdir dummy
# The --target flag is important because it prevents build.rs scripts from being built with
# the above-specified RUSTFLAGS.
cargo run --target-dir ./coverage --release --target $TARGET --bin "fuzz_target" -- -merge=1 ${@:1} `pwd`/dummy `pwd`/corpus
rm -rf dummy
# rustup component add llvm-tools-preview cargo-binutils 
# rust-profdata merge ./test.profraw -o test.profdat
# rust-cov report -instr-profile=test.profdata ./coverage/x86_64-unknown-linux-gnu/release/fuzz_target
